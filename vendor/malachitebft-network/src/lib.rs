...
async fn handle_ctrl_msg(
    ...
    match msg {
        ...
        CtrlMsg::SyncRequest(peer_id, request, reply_to) => {
            ...
            let request_id = sync.send_request(peer_id.to_libp2p(), request.clone());

            info!(
                %request_id,
                %peer_id,
                request_size = request.len(),
                "Sending sync request to peer"
            );
    ...
    match event {
        ...
        sync::Event::OutboundFailure {
            ...
            warn!(
                ...
                peer = %peer,
                error = ?error,
                "Sync outbound request failed"
            );

            // Send a None response to trigger retry in the sync layer
            // This notifies the sync engine that the request failed
            let _ = tx_event
                .send(Event::Sync(sync::RawMessage::Response {
                    request_id,
                    peer: PeerId::from_libp2p(&peer),
                    body: Bytes::new(), // Empty body will be decoded as None
                }))
                .await
                .map_err(|e| {
                    error!("Error sending Sync failure response to handle: {e}");
                });

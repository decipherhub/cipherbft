    ...
    Ctx: Context,
{
    ...
    if height >= limit {
        state.consecutive_blocked_count += 1;

        // If we've been blocked for too many iterations (60 = ~30 seconds at 500ms interval),
        // clear all pending requests and retry. This handles cases where libp2p timeouts
        // aren't triggering for some reason.
        const MAX_BLOCKED_ITERATIONS: u32 = 60;

        if state.consecutive_blocked_count >= MAX_BLOCKED_ITERATIONS {
            warn!(
                height.sync = %state.sync_height,
                height.limit = %limit,
                pending_requests = state.pending_value_requests.len(),
                blocked_iterations = state.consecutive_blocked_count,
                "SYNC STUCK: Clearing all pending requests after {} blocked iterations",
                state.consecutive_blocked_count
            );
            state.clear_all_pending_requests();
            // Don't return - fall through to try requesting again
        } else {
            warn!(
                height.sync = %state.sync_height,
                height.limit = %limit,
        ...
        // Reset blocked counter when we can make progress
        state.consecutive_blocked_count = 0;
    }
